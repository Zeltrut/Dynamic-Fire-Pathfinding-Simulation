...
## Overview
Our project simulates a building on fire, with available paths dynamically altered. We compare **A* (A-Star)** as an informed search and **UCS (Uniform Cost Search)** as a uniform baseline to see which finds a better path from start to goal as the grid changes.
